...
String _sourceLabel(EnergyDataSource s) => switch (s) {
      EnergyDataSource.live => 'Live',
      EnergyDataSource.offline => 'Estimated (offline)',
      EnergyDataSource.loading => 'Syncing',
    };

Color _sourceColor(EnergyDataSource s) => switch (s) {
...
class _SettingsPageState extends State<SettingsPage> {
          ...
          children: [
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    Text('Changeover control, baselines & manual calibration',
                ...
                Container(
                  ...
                  child: Text('v1.0.0',
                      style: AppType.mono(9, color: AppColors.textMuted)),
                ),
              ],
            ),
            ...
            GlassCard(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // Header row: dot+label + status pill
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      _dotLabel('BACKEND SYNC', color: AppColors.info),
                      _pill(
                          _sourceLabel(p.source).toUpperCase(),
                          _sourceColor(p.source),
                          filled: p.source == EnergyDataSource.live),
                    ],
                  ),
                  const SizedBox(height: 10),
                  // Hero value
                  Text(_sourceLabel(p.source),
                      style: const TextStyle(
                          fontFamily: 'Outfit',
                          fontSize: 22,
                          fontWeight: FontWeight.w700,
                  ...
                  Text(
                      p.lastSync == null
                          ? 'Never synced'
                          : 'Last sync ${_fmtLastSync(p.lastSync!)}',
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      _dotLabel('ACTIVE SOURCE', color: AppColors.home),
                      _pill('ACTIVE', AppColors.home, filled: true),
                    ],
                  ),
                  const SizedBox(height: 10),
                  Text(active.label,
                  ...
                  const SizedBox(height: 3),
                  Text(
                      active.isDigital ? 'Digital meter · WAPDA grid' : 'Analog meter · Solar',
                      style: AppType.inter(9, color: AppColors.textSecondary)),
                  const SizedBox(height: 10),
                  _overlay(
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('Switch to the other meter',
                            style: AppType.inter(9,
                                color: AppColors.textSecondary)),
                        GestureDetector(
                          onTap: p.changeover,
                          child: Container(
                            padding: const EdgeInsets.symmetric(
                                horizontal: 10, vertical: 5),
                            decoration: BoxDecoration(
                              color: AppColors.home.withValues(alpha: 0.16),
                              borderRadius: BorderRadius.circular(6),
                            ),
                            child: Row(
                              mainAxisSize: MainAxisSize.min,
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      _dotLabel('BILLING ANCHOR', color: AppColors.info),
                      _pill('28TH', AppColors.info),
                    ],
                  ),
                  ...
                  const SizedBox(height: 3),
                  Text(
                      'Physical readings from your bill that anchor the current monthly cycle.',
                      style: AppType.inter(8.5,
                          color: AppColors.textMuted, height: 1.4)),
                  const SizedBox(height: 12),
                  // Overlay inputs block
                  _overlay(
                    ...
                    child: _editingBaselines
                        ? FilledButton.icon(
                            ...
                            style: FilledButton.styleFrom(
                              ...
                              textStyle:
                                  AppType.inter(10, weight: FontWeight.w700),
                            ),
                          )
                        : OutlinedButton.icon(
                            onPressed: () =>
                                setState(() => _editingBaselines = true),
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      _dotLabel('LOG READINGS', color: AppColors.purple),
                      _pill('2 METERS', AppColors.purple),
                    ],
                  ),
                  const SizedBox(height: 8),
                  ...
                  Text(
                      'Enter the latest physical readings from both meters.',
                      style: AppType.inter(8.5,
                          color: AppColors.textMuted, height: 1.4)),
                  const SizedBox(height: 12),
                  _overlay(
                    ...
                    child: Column(
                      children: [
                        _ReadingInput(
                            label: 'Meter 1 (Analog)',
                            controller: _m1Controller,
                            lastLogged: m1.lastLoggedReading ?? 0,
                            lastTs: m1.lastLoggedAt ?? 0),
                        const SizedBox(height: 10),
                        _ReadingInput(
                            label: 'Meter 2 (Digital)',
                            controller: _m2Controller,
                            lastLogged: m2.lastLoggedReading ?? 0,
                            lastTs: m2.lastLoggedAt ?? 0),
                      ],
                    ),
                  ),
                  const SizedBox(height: 10),
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      _dotLabel('LAST BILLING CYCLE', color: AppColors.home),
                      _pill(
                          p.home.vsLastMonthPercent == null
                              ? 'NOT SET'
                              : (p.home.vsLastMonthPercent! <= 0
                                  ? 'SAVING'
                                  : 'OVER'),
                          p.home.vsLastMonthPercent == null
                              ? AppColors.textMuted
                  ...
                  Text.rich(
                    TextSpan(
                      text: p.home.lastMonthTotal == null
                          ? '--'
                          : p.home.lastMonthTotal!.round().toString(),
                      style: const TextStyle(
                          fontFamily: 'Outfit',
                          fontSize: 26,
                          fontWeight: FontWeight.w700,
                  ...
                  const SizedBox(height: 2),
                  Text(
                      'Total units used last month. Overrides auto-calculated value for trend comparison.',
                      style: AppType.inter(8.5,
                          color: AppColors.textMuted, height: 1.4)),
                  const SizedBox(height: 12),
                  _overlay(
                  ...
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      onPressed: () {
                        final v =
                            double.tryParse(_lastMonthController.text.trim());
            ...
            // ── Scene overlay editor (dev-mode line positioning) ────
            const SectionHeader(
                icon: Icons.edit_rounded,
                color: AppColors.purple,
                title: 'Scene Overlay Editor'),
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      _dotLabel('LIVE DATA OVERLAY',
                          color: AppColors.warning),
                      _pill(
                          p.overlayEnabled ? 'ON' : 'OFF',
                          p.overlayEnabled
                              ? AppColors.warning
                              : AppColors.textMuted,
                          filled: p.overlayEnabled),
                    ],
                  ),
                  const SizedBox(height: 10),
                  Text(p.overlayEnabled ? 'Overlay active' : 'Overlay disabled',
                      style: TextStyle(
                          fontFamily: 'Outfit',
                          fontSize: 18,
                          fontWeight: FontWeight.w700,
                          color: p.overlayEnabled
                              ? AppColors.warning
  ...
  /// Small dot + uppercase label, matching the Solar MeterCard headers.
  ...
  /// Compact status pill (filled or tinted).
  ...
  /// Overlay background block — the subtle white tint used inside Solar cards.
  Widget _overlay({
  ...
  Widget build(BuildContext context) {
    ...
    final color = isNeutral
        ...
        : isLower
            ? AppColors.purple
            : AppColors.info;
    final frac = (isLower ? pct.abs() : -pct.abs()).clamp(-1.0, 1.0) / 2 +
        0.5; // 0..1, left=low
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 4),
                  Text(
                      '${isLower ? '-' : '+'}${pct.abs().toStringAsFixed(0)} units',
                      style: AppType.inter(11,
                          color: AppColors.textPrimary,
                          weight: FontWeight.w700)),
                ],
              ...
              Text(label,
                  style:
                      AppType.inter(9, color: color, weight: FontWeight.w700)),
...
class _SemiGaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(rect, pi, pi, false,
        track..color = AppColors.info.withValues(alpha: 0.25));
    canvas.drawArc(rect, 0, pi, false,
        track..color = AppColors.purple.withValues(alpha: 0.25));
